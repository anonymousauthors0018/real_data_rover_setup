...
class ArFollower():
	def __init__(self):
		...
		self.goal_x = rospy.get_param("~goal_x")#此处x、y方向为ROS方向
		...
		self.angularleft_p = 2.0
		self.angularright_p =2.0
		self.d_param = 0.5
		
		#订阅AR标签位姿信息，发布速度话题
		self.cmd_vel_pub = rospy.Publisher("/cmd_vel", Twist, queue_size=5)
		self.ar_pose_sub = rospy.Subscriber("/aruco_single/marker", Marker,self.set_cmd_vel )
	...
	def set_cmd_vel(self, msg):
		...
		offset_x = 0.0 #小车中心与摄像头检测到的AR标签中心的偏差
		target_offset_x = msg.pose.position.x + offset_x #AR标签位姿信息x方向(已校正)-对应ROS中y方向
		#rospy.loginfo(target_offset_x)
		rospy.loginfo("get target")
		target_offset_z = msg.pose.position.z #AR标签位姿信息z方向-对应ROS中x方向

		#当AR标签和小车的距离与设定距离存在偏差时
		if target_offset_z > self.goal_x:
			linearspeed = target_offset_z * self.linearfront_p - self.d_param * 0.6
			...
			#当AR标签中心与小车中心存在偏差时
			if target_offset_x > self.goal_y: 
				angularspeed = -target_offset_x * self.angularright_p - self.d_param * 0.1
				if abs(angularspeed) < 0.005:
					angularspeed = 0
					#极低速置零，避免小车摇摆
				...
				angularspeed = -target_offset_x * self.angularleft_p + self.d_param * 0.1
				if abs(angularspeed) < 0.005:
					angularspeed = 0
				if abs(angularspeed) > self.max_angular_speed:
					angularspeed = self.max_angular_speed
			...
			linearspeed = (target_offset_z - self.goal_x) * self.linearback_p + self.d_param * 0.1

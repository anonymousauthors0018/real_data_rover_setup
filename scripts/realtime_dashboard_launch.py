#!/usr/bin/env python3
"""
Real-time Sync Viewer — MJPEG HTTP stream + Launch Control
Subscribes to /synced/pair, /camera/color, /camera/depth, renders the sync panel,
serves it as MJPEG at http://<jetson-ip>:8080, and allows starting/stopping a ROS2
launch file from the dashboard UI.
...
# Replace these with your real ROS2 package + launch file
LAUNCH_PACKAGE = "sensor_bringup"
LAUNCH_FILE = "sensors_dashboard.launch.py"

ROS_SETUP = "/opt/ros/foxy/setup.bash"
WS_SETUP = "/workspace/ros_ws/install/setup.bash"
LAUNCH_LOG = "/workspace/tmp/dashboard_launch.log"
...
def build_launch_cmd(object_name: str, duration: str):
    safe_name = clean_name(object_name)
    safe_duration = clean_duration(duration)

    cmd = (
        f"source {ROS_SETUP} && "
        f"source {WS_SETUP} && "
        f"ros2 launch {LAUNCH_PACKAGE} {LAUNCH_FILE} "
        f"object_name:={safe_name} "
        f"record_duration_sec:={safe_duration}"
    )
    return ["bash", "-lc", cmd], safe_name, safe_duration
...
class LaunchManager:
    def __init__(self):
        self.lock = threading.Lock()
        self.proc = None
        self.last_object_name = "unknown_object"
        self.last_duration = "15.0"
        # Store the last 100 lines of logs in memory
    ...
    def _read_stream(self, stream):
        """
        Reads the subprocess output line by line and stores it in the deque.
        ...
        try:
            ...
            with self.lock:
                self.logs.append(f"LOG ERROR: {e}")
        finally:
            stream.close()

    def start(self, object_name, duration):
        with self.lock:
            if self.proc is not None and self.proc.poll() is None:
                return False, "Launch file already running"

            try:
                cmd, safe_name, safe_duration = build_launch_cmd(object_name, duration)
                
                self.logs.clear()
                self.logs.append(f"--- Launching: {safe_name} ---")

                # We use PIPE for stdout and redirect stderr to stdout so we catch everything
                ...
                self.last_object_name = safe_name
                self.last_duration = safe_duration
                return True, f"Started {safe_name}"
            except Exception as e:
                self.proc = None
                return False, f"Failed to start launch: {e}"

    def stop(self):
        with self.lock:
            if self.proc is None or self.proc.poll() is not None:
                return False, "No launch file is running"

            try:
                self.proc.terminate()
                self.logs.append("--- Termination signal sent ---")
                self.proc = None
                return True, "Launch file stopped"
            except Exception as e:
                return False, f"Failed to stop launch: {e}"
...
class MJPEGHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/':
            ...
            <head>
                ...
                <style>
                    ...
                    .log-card {
                        width: 100%;
                        background: #0f172a; /* Dark terminal background */
                        color: #38bdf8; /* Terminal blue/cyan text */
                        padding: 1rem;
                        border-radius: 12px;
                        margin-top: 1rem;
            ...
            <body>
                    ...
                    <div class="control-card">
                        <div class="control-row">
                            <div class="input-group">
                                ...
                                <input id="objectName" type="text" placeholder="e.g. chair" />
                            </div>

                            <div class="input-group">
                                <label for="durationSec">Duration (seconds)</label>
                                <input id="durationSec" type="number" min="1" step="0.1" value="15.0" />
                            </div>

                            <div class="button-group">
                                <button class="start-btn" onclick="startLaunch()">Start Launch</button>
                                <button class="stop-btn" onclick="stopLaunch()">Stop Launch</button>
                            </div>
                        </div>

                        <div id="launchStatus" class="launch-status">Checking status...</div>
                    </div>

                    <div class="log-card" id="logWindow">
                ...
                <script>
                    async function refreshLaunchStatus() {
                        try {
                            const res = await fetch('/launch/status');
                            const data = await res.json();

                            let text = data.running ? 'Launch status: RUNNING' : 'Launch status: STOPPED';
                            text += ' | Object: ' + (data.object_name || 'unknown_object');
                            text += ' | Duration: ' + (data.duration || '15.0') + 's';

                            document.getElementById('launchStatus').textContent = text;
                        } catch (e) {
                            document.getElementById('launchStatus').textContent = 'Launch status: ERROR';
                        }
                    }

                    async function startLaunch() {
                        const objectName = encodeURIComponent(
                            document.getElementById('objectName').value || 'unknown_object'
                        );
                        const duration = encodeURIComponent(
                            document.getElementById('durationSec').value || '15.0'
                        );

                        const res = await fetch(`/launch/start?object_name=${objectName}&duration=${duration}`);
                        const data = await res.json();
                        alert(data.message);
                        refreshLaunchStatus();
                    }
                    ...
                    async function stopLaunch() {
                        const res = await fetch('/launch/stop');
                        const data = await res.json();
                        alert(data.message);
                        refreshLaunchStatus();
                    }
                    ...
                    async function refreshLogs() {
                        try {
                            const res = await fetch('/launch/logs');
                            const data = await res.json();
                            const logWindow = document.getElementById('logWindow');
                            
                            // Only update if there are logs to show
        ...
        elif path == '/launch/start':
            qs = parse_qs(parsed.query)
            object_name = qs.get("object_name", ["unknown_object"])[0]
            duration = qs.get("duration", ["15.0"])[0]
            ok, msg = LAUNCH_MANAGER.start(object_name, duration)
            self._send_json({
                "ok": ok,
                "message": msg,
        ...
        elif path == '/launch/stop':
            ok, msg = LAUNCH_MANAGER.stop()
            self._send_json({
                "ok": ok,
                "message": msg,
                **LAUNCH_MANAGER.status()
            ...
            self._send_json(LAUNCH_MANAGER.status())

        elif path == '/launch/logs':
            self._send_json({"logs": LAUNCH_MANAGER.get_logs()})
